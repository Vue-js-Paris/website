<template>
  <ul class="flex items-center gap-5 text-3xl">
    <li
      class="transtion-all hover:scale-125 hover:text-primary hover:duration-300 hover:ease-in-out"
    >
      <a href="https://www.youtube.com/@vuejsparis" target="_blank"
        ><Icon name="mingcute:youtube-fill"
      /></a>
    </li>
    <li
      class="transtion-all hover:scale-125 hover:text-primary hover:duration-300 hover:ease-in-out"
    >
      <a href="https://x.com/vuejsparis" target="_blank"
        ><Icon name="ri:twitter-x-fill"
      /></a>
    </li>
    <li
      class="transtion-all hover:scale-125 hover:text-primary hover:duration-300 hover:ease-in-out"
    >
      <a href="https://bsky.app/profile/vuejsparis.bsky.social" target="_blank"
        ><Icon name="ri:bluesky-fill"
      /></a>
    </li>
    <li
      class="transtion-all hover:scale-125 hover:text-primary hover:duration-300 hover:ease-in-out"
    >
      <a href="https://www.meetup.com/fr-FR/Vuejs-Paris/" target="_blank"
        ><Icon name="cib:meetup"
      /></a>
    </li>
  </ul>
</template>
